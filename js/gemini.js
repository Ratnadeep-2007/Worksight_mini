...
"interviewSentiment" (string: "Very Positive" | "Positive" | "Neutral" | "Negative")
"initialEngagement" (integer 0-100)
"meetingsPerWeek" (integer)
"meetingsBaseline" (integer)
"backToBackPct" (integer 0-100)
"lateNightLogins" (integer)
"weekendLogins" (integer)
"taskCompletionCurrent" (integer 0-100)
"taskCompletionBaseline" (integer 0-100)
"avgTaskTime" (number)
"msgFrequency" (integer)
"msgFrequencyBaseline" (integer)
"responseLatency" (integer)
"responseLatencyBaseline" (integer)
"teamInteractionScore" (integer 0-100)
"teamInteractionBaseline" (integer 0-100)
"isolationFlag" (string: "Yes" | "No")
Return ONLY the raw JSON object, no markdown blocks.`;

async function parseTranscriptWithGemini(transcript, apiKey, model) {
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`;
  const payload = {
    system_instruction: { parts: [{ text: TRANSCRIPT_SYSTEM_PROMPT }] },
